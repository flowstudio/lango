...
module.exports = class I18n {
	...
	configure(options = {}) {
		...
		this.defaultLocale = options.defaultLocale || Object.key(this.locales)[0];
	...
	translate(text, count) {
		...
		let translation = localeData.getTranslation(text);

		if (!translation) {
			localeData = this.getLocale(this.getFallbackLocale(locale));

			if (localeData.locale !== locale) {
				translation = localeData.getTranslation(text);
			}
		} else {
			isTranslated = true;
		}
